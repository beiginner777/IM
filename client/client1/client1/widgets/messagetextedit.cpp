...
QVector<std::shared_ptr<MsgInfo>> MessageTextEdit::getMsgList()
{
    ...
    for(int index=0; index<doc.size(); index++)
    {
        ...
        if(doc[index]==QChar::ObjectReplacementCharacter)
        {
            if(!text.isEmpty())
            {
                insertMsgList(_total_msg_list, CHAT_MSG_TYPE::TEXT_MSG, text, QPixmap(),"",0,"");
                text.clear();
            }
            while(indexUrl<count)
            {
                std::shared_ptr<MsgInfo> msg = _img_or_file_list[indexUrl];
                if(this->document()->toHtml().contains(msg->text_or_url_,Qt::CaseSensitive))
                {
                    indexUrl++;
...
void MessageTextEdit::insertImages(const QString &url)
{
    ...
    insertMsgList(_img_or_file_list, CHAT_MSG_TYPE::PIC_MSG, url, QPixmap::fromImage(image), unique_name,
        total_size, fileMd5);
}
...
void MessageTextEdit::insertFiles(const QString& url) {
    ...
    QString origin_name = fileInfo.fileName();
    QString unique_name = generateUniqueFileName(origin_name);
    insertMsgList(_img_or_file_list, CHAT_MSG_TYPE::FILE_MSG, url, pix, unique_name,
        total_size, fileMd5);
}
...
void MessageTextEdit::insertMsgList(QVector<std::shared_ptr<MsgInfo>> &list, CHAT_MSG_TYPE msgtype,
    ...
    if(msgtype == CHAT_MSG_TYPE::PIC_MSG || msgtype == CHAT_MSG_TYPE::FILE_MSG) {
        msg_info->_type = TRANSFER_TYPE::Upload;
