...
n	std::map<std::string, std::shared_ptr<CSession>> copy_resource_sessions_;
	{
		std::lock_guard<std::mutex> lcoker(mtx_);
	...
	for (auto it = copy_sessions_.begin(); it != copy_sessions_.end(); ++it) {
			...
			if (expired) {
				...
				// 过期连接
					for (auto& [uuid, session] : copy_resource_sessions_) {
		if (session->isHeartOverTime()) {
...
void CServer::moveToResourceSessions(const std::string& uuid)
{
	std::lock_guard<std::mutex> locker(mtx_);
	auto it = sessions_.find(uuid);
	if (it != sessions_.end()) {
		resource_sessions_[uuid] = it->second;
		sessions_.erase(it);
		std::cout << "[CServer] Moved session " << uuid << " to resource_sessions_" << std::endl;
	}
}
