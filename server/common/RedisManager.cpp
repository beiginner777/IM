...
bool RedisManager::SetBit(const std::string& key, size_t offset, int value)
{
	auto connect_ = getConn(true);
	if (connect_ == nullptr) {
		std::cerr << "SetBit: Get RedisConn failed." << std::endl;
		return false;
	}
	Defer defer([this, &connect_]() {
		returnConn(connect_);
	});
	redisReply* reply = (redisReply*)redisCommand(connect_, "SETBIT %s %zu %d",
...
int RedisManager::GetBit(const std::string& key, size_t offset)
{
	...
	if (connect_ == nullptr) {
		std::cerr << "GetBit: Get RedisConn failed." << std::endl;
		return -1;
	}
	Defer defer([this, &connect_]() {
		returnConn(connect_);
	});
	redisReply* reply = (redisReply*)redisCommand(connect_, "GETBIT %s %zu",
		key.c_str(), offset);
	if (!reply || reply->type != REDIS_REPLY_INTEGER) {
		if (reply) freeReplyObject(reply);
		return -1;
	}
	int val = (int)reply->integer;
	freeReplyObject(reply);
	return val;
}
