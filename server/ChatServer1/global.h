...
enum ERROE_CODR
{
    ...
    ERROR_LOAD_CHAT_THREAD, // 加载用户的聊天列表失败
    ERROR_CREATE_PRIVATE_THREAD, // 创建私聊线程失败
    ERROR_SEND_MSG_FAILED, // 发送消息失败
	ERROR_LOAD_MORE_FRIEND, // 加载更多好友失败
	ERROR_PUSH_NOTIFY_MESSAGE, // 推送通知消息失败
	ERROR_GET_NOTIFY_MESSAGE, // 获取通知消息失败
	ERROR_LOAD_FRIEND_APPLY, // 加载好友申请失败
    ERROR_USER_NOT_EXIST_IN_CHATSERVER, // 当ResourceServer调用的时候，发现用户不存在
    ERROR_CHATIMG_NOT_EXIST_IN_CHATSERVER,// 当ResourceServer调用的时候，发现ChatImageMsg不存在
    ERROR_MODIFY_MSG_STATUS, // 修改聊天消息的状态失败
    ERROR_LOAD_CHAT_MESSAGE, // 加载聊天消息失败
    SUCCESS = 0,
};
...
enum REQUEST_ID
{
    ...
    ID_NOTIFY_CHAT_IMAGE_MSG, // 通知有图片消息

    ID_LOAD_CHAT_MSG_REQ, // 加载聊天消息的请求
    ID_LOAD_CHAT_MSG_RSP, // 加载聊天消息的回包

	ID_REGISTER_REQ, // 向StatusServer注册的消息
	ID_REGISTER_RSP // 向StatusServer注册的回包
};

enum REDIS_ID
{
	REDIS_ID_FRIEND_ICON_CHANGE, // 用户头像变更
};
