...
HttpConnection::HttpConnection(tcp::socket&& sock, SeckillServer* server)
    : sock_(std::move(sock)), deadline_(sock_.get_executor(), std::chrono::seconds(60)), server_(server),
...
void HttpConnection::resetDeadline()
{
	...
	deadline_.async_wait(
	        ...
	        {
		        if (ec)
			        return; // operation_aborted：主动取消了，不做任何事
...
{
	auto self = shared_from_this();
	http::async_read(sock_, buffer_, request_,
