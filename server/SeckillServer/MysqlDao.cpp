	...
	try
	{
		...
		while (res->next())
		{
			result.push_back({res->getInt("id"), res->getString("name"), (double) res->getDouble("price"),
			                  res->getInt("stock"), res->getString("image_url")});
		}
	}
	catch (sql::SQLException& e)
	{
		std::cerr << "[MysqlDao] getProducts: " << e.what() << std::endl;
	}
	returnConn(std::move(conn));
	return result;
}
...
bool MysqlDao::updateStock(int productId, int newStock)
{
	...
	{
		std::cerr << "[MysqlDao] updateStock: " << e.what() << std::endl;
		returnConn(std::move(conn));
		return false;
	}
	returnConn(std::move(conn));
...
int MysqlDao::insertOrder(int uid, int productId, const std::string& productName, double price)
{
	...
	try
	{
		auto stmt = conn->con_->prepareStatement(
		        "INSERT INTO seckill_order(uid,product_id,product_name,price,status) VALUES(?,?,?,?,'unpaid')");
		stmt->setInt(1, uid);
		stmt->setInt(2, productId);
	...
	{
		std::cerr << "[MysqlDao] insertOrder: " << e.what() << std::endl;
		returnConn(std::move(conn));
		return -1;
	}
}
...
std::vector<MysqlDao::Order> MysqlDao::getOrdersByUid(int uid)
{
	...
	{
		auto stmt =
		        conn->con_->prepareStatement("SELECT id,uid,product_id,product_name,price,status,recipient,created_at "
		                                     "FROM seckill_order WHERE uid=? ORDER BY id DESC");
		stmt->setInt(1, uid);
	...
	{
		std::cerr << "[MysqlDao] getOrdersByUid: " << e.what() << std::endl;
	}
	returnConn(std::move(conn));
	return result;
}
...
std::string MysqlDao::getUsername(int uid)
{
	...
	{
		std::cerr << "[MysqlDao] getUsername: " << e.what() << std::endl;
	}
	returnConn(std::move(conn));
	return "";
}
...
bool MysqlDao::payOrder(int orderId, int uid)
{
	...
	{
		auto stmt = conn->con_->prepareStatement(
		        "UPDATE seckill_order SET status='paid' WHERE id=? AND uid=? AND status='unpaid'");
	...
	{
		std::cerr << "[MysqlDao] payOrder: " << e.what() << std::endl;
		returnConn(std::move(conn));
		return false;
	}
}
...
bool MysqlDao::cancelOrder(int orderId, int uid)
{
	...
	{
		auto stmt = conn->con_->prepareStatement(
		        "UPDATE seckill_order SET status='cancelled',cancelled_at=NOW() WHERE id=? AND uid=?");
		stmt->setInt(1, orderId);
		stmt->setInt(2, uid);
		stmt->executeUpdate();
		returnConn(std::move(conn));
	...
	{
		std::cerr << "[MysqlDao] cancelOrder: " << e.what() << std::endl;
		returnConn(std::move(conn));
		return false;
	}
}
...
MysqlDao::Order MysqlDao::getOrderById(int orderId)
{
	...
	try
	{
		auto stmt = conn->con_->createStatement();
		auto res = stmt->executeQuery(
		        "SELECT id,uid,product_id,product_name,price,status,recipient,created_at FROM seckill_order WHERE id=" +
	...
	{
		std::cerr << "[MysqlDao] getOrderById: " << e.what() << std::endl;
	}
	returnConn(std::move(conn));
	return o;
}
